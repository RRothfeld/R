...
# Regression 
# HH autos = fn(HH size, HH income, Number of workers, Number of children, 
# Distance to transit, Region type)

# Check correlation between independent variables
cor1 <- cor(subset(sampleData,
				   select = c("hh_size", "hh_income", "n_emp",
				   		   "n_children", "dist_to_transit", "region_type")))

head(subset(sampleData,
			select = c("hh_size", "hh_income", "n_emp", "n_children",
					   "dist_to_transit", "region_type")))
...
library(corrplot)
corrplot(cor1, method = "number")	# Whhich variables are strongly correlated?
...
# Interaction term
# a*b --> a+b+a:b
regressionFit5 <- lm(hh_autos~hh_size+hh_income+n_emp+dist_to_transit*region_type,
					 data = sampleData,
					 weights = sampleData$weight)
summary(regressionFit5)

# a:b --> (only) a:b
regressionFit6 <- lm(hh_autos~hh_size+hh_income+n_emp+dist_to_transit:region_type,
					 data = sampleData,
					 weights = sampleData$weight)
summary(regressionFit6)

# Save predictions of a model in a new data frame together with variables you
